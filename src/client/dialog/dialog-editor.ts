 ...
 */
function createMentionElement(label, options: any = {}) {
 ...
 */
export function createDialogEditor(options: any = {}) {
    ...
    return {
        ...
        hasReference(inspPath) {
            if (!editorEl || !inspPath)
                return false;
            return Array.from(editorEl.querySelectorAll('.cii-mention')).some((node: any) => node.dataset.inspPath === inspPath);
        },
