 ...
 * the types describe the intended config surface for editors and `tsc`.
 */
...
/**
 * Vite entry (also the package default). Returns `[codeInspector, inspector]`
 * so both register as separate Vite plugins.
 */
export function vite(options?: IdeByebyeOptions): PluginInstance[];

/** webpack entry — single plugin that also registers code-inspector. */
...
/** rsbuild entry — `plugins: [inspector()]`. */
export function rsbuild(options?: IdeByebyeOptions): PluginInstance;

/** esbuild entry — `[codeInspector, inspector]`; may use `htmlFiles`. */
export function esbuild(options?: IdeByebyeOptions): PluginInstance[];

/** Farm entry — `[codeInspector, inspector]`. */
export function farm(options?: IdeByebyeOptions): PluginInstance[];

/**
 * Turbopack (Next.js) rules only (`data-insp-path`). Bootstrap must be mounted
 * by the app — see README.
 */
export function turbopack(options?: IdeByebyeOptions): PluginInstance;

/**
 * Mako (Umi) path injection only. Same bootstrap caveat as {@link turbopack}.
 */
export function mako(options?: IdeByebyeOptions): PluginInstance;

/**
 * Back-compatible default: Vite entry (`plugins: [codeIntentInspectorPlugin()]`).
 */
export const codeIntentInspectorPlugin: typeof vite;

/**
 * Default export is the Vite adapter (same as `codeIntentInspectorPlugin`).
 *
 * @example
 * ```ts
 * import ideByebye from 'ide-byebye';
 * export default defineConfig({ plugins: [ideByebye()] });
 * ```
 */
declare const ideByebye: typeof vite;
export default ideByebye;
